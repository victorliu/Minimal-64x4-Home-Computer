...
OS_Start:       0xff                                          ; switch OFF FLASH after boot-up
  ...
  OS_Splash:    JPS OS_SerialPrint                            ; send "READY." via UART
                ...
                MIZ 20,_XPos JPS OS_Print "**** MINIMAL 64x4 - MinOS 2 ****", 10, 10, 0
                MIZ 20,_XPos JPS OS_Print "64KB SRAM - 512KB FLASH - 8.0MHz", 10, 10, 0
                MIZ 20,_XPos JPS OS_Print "Type 'show manual' for more info", 10, 10, 0
...
; modifies: Z0
; ------------------------------------------------------------
OS_ClearRow:    LDI <BYTES_PER_ROW+1 SUZ _XPos LR1 FEQ cr_exit        ; calculate number of words to clear
                  STB cr_line+1
...
OS_ScrollDn:    MIB 0x7b,sd_loopx+2                           ; set source and target page
  ...
  sd_loopq:     MIZ <BYTES_PER_ROW,Z1                         ; chars per row
  sd_loopx:     LZP Z0,0xcc SZP Z0,0xcc                       ; move a char (upper and lower part)
                ...
                INZ Z0 DEZ Z1 FGT sd_loopx                    ; horizontal step
                  AIZ <64-BYTES_PER_ROW,Z0 FCC sd_loopq       ; next line, repeat 4 times
...
; modifies: Z0..2
; *******************************************************************************
OS_Char0:        STZ Z0 LDI <VIEWPORT ADZ _XPos STZ Z1         ; index to video position of char
...
OS_Char:
Font6x8_DrawChar:
	STZ Z0                    ; Store the character
	; Stuff the character table address
	MIW Font6x8_CharTable, Font6x8_DrawChar_getchar+1
	AZW Z0, Font6x8_DrawChar_getchar+1

	; Since each character is 6 pixels/bits wide, we will not always land within
	; a byte; we may have to straddle a byte boundary.
	; The viewport row[i] starts at byte VIEWPORT + 64*i andn goes on for 50 bytes.
	; The pixel at (j,i) is within byte: VIEWPORT + 64*i + (j/8)
	; Of each set of 4 successive pixels, the first and last will be completely within
	; a byte, while the middle two straddle a byte boundary.
	;
	; The pixels of row[i] of character with ASCII code k is at byte:
	;   Font6x8_CharTable + 128*i + k
	; in the lowest 6 bits.
	;
	; Finally, for a character that will be placed at (XPos, YPos), the address
	; in the viewport would be VIEWPORT + 64*YPos + XPos * 6/8.
	;
	; msb |DDDDDDCC|CCCCBBBB|BBAAAAAA| lsb
	; A = leftmost of 4 characters
	; D = rightmost of 4 characters
	;
	; XPos | Row byte offset
	; -----+----------------
	;  0   |   0
	;  1   |   0,1
	;  2   |   1,2
	...
	; switch(XPos % 4){
	; case 0:
	;     byte VIEWPORT[64*YPos + XPOS * 3 / 4] &= 0xC0;
	;     byte VIEWPORT[64*YPos + XPOS * 3 / 4] |= CharTable[128*YPos + k];
	; case 1:
	;     word VIEWPORT[64*YPos + (XPOS-1) * 3 / 4] &= 0xF03F;
	;     word VIEWPORT[64*YPos + (XPOS-1) * 3 / 4] |= CharTable[128*YPos + k] << 6;
	; case 2:
	;     word VIEWPORT[64*YPos + (XPOS-2) * 3 / 4 + 1] &= 0xFC0F;
	;     word VIEWPORT[64*YPos + (XPOS-2) * 3 / 4 + 1] |= CharTable[128*YPos + k] << 4;
	; case 3:
	;     byte VIEWPORT[64*YPos + (XPOS-3) * 3 / 4 + 2] &= 0x03;
	;     byte VIEWPORT[64*YPos + (XPOS-3) * 3 / 4 + 2] |= CharTable[128*YPos + k] << 2;
	; }
	;
	; More generically, using words:
	;     word VIEWPORT[64*YPos + XPOS/4*3 + VOFF] &= MASK;
	;     word VIEWPORT[64*YPos + XPOS/4*3 + VOFF] |= CharTable[128*YPos + k] << SHIFT;
	; where:
	;   XPos % 4 | VOFF |  MASK  | SHIFT
	;   ---------+------+--------+------
	;        0   |   0  | 0xFFC0 |   0
	;        1   |   0  | 0xF03F |   6
	;        2   |   1  | 0xFC0F |   4
	...
	LDZ _XPos  ANI 0x03  STZ Z0

	LZB Z0, Font6x8_DrawChar_SHIFT_table   STZ Z2
		; Stuff address
		MIW Font6x8_DrawChar_shiftdone, Font6x8_DrawChar_shifts+1
		LDZ Z2  LL1
		SUW Font6x8_DrawChar_shifts+1

	LZB Z0, Font6x8_DrawChar_VOFF_table    STB Font6x8_DrawChar_AddVOFF+1
	LZB Z0, Font6x8_DrawChar_MASKLO_table  STB Font6x8_DrawChar_MASKLO+3
	LZB Z0, Font6x8_DrawChar_MASKHI_table  STB Font6x8_DrawChar_MASKHI+3

	LDZ _XPos  LR2  STZ Z0  LL1  ADZ Z0      ; A = XPos / 4 * 3
	Font6x8_DrawChar_AddVOFF: ADI 0xcc       ; Add VOFF
	ADI <VIEWPORT STZ Z6
	LDZ _YPos LL1 ADI >VIEWPORT STZ Z7   ; multiply y with 8*64 = 512
	; Z6,Z7 is the viewport address

	MIZ 8, Z2
	Font6x8_DrawChar_rowloop:
		; Get character row
		Font6x8_DrawChar_getchar: RDB 0xcccc,1  STZ Z0  CLZ Z1

		; Shift the word by required amount
		CLC
		Font6x8_DrawChar_shifts:  JPA 0xcccc
		RLV Z0  RLV Z0
		RLV Z0  RLV Z0
		RLV Z0  RLV Z0
		Font6x8_DrawChar_shiftdone:
		
		; Load existing viewport word, mask off, OR in, store
		Font6x8_DrawChar_MASKLO: LDT Z6  ANI 0xcc  ORZ Z0  STT Z6 INZ Z6
		Font6x8_DrawChar_MASKHI: LDT Z6  ANI 0xcc  ORZ Z1  STT Z6

		; Increment CharTable address
		INB Font6x8_DrawChar_getchar+2

		; Increment viewport address (we already incremented once above)
		AIV 63, Z6

		DEZ Z2  BNE Font6x8_DrawChar_rowloop

	RTS

Font6x8_DrawChar_VOFF_table: 0, 0, 1, 2
Font6x8_DrawChar_MASKLO_table: 0xC0, 0x3F, 0x0F, 0x03
Font6x8_DrawChar_MASKHI_table: 0xFF, 0xF0, 0xFC, 0xFF
Font6x8_DrawChar_SHIFT_table: 0, 6, 4, 2
...
; The VRAM is a region 16KB in size, which is 512x256 bits.
; The viewport is a subset that starts at pixel row index 12, pixel column index 88.
; Each viewport row is 50 bytes long, with a stride of 64 bytes.
...
#org 0x0042     WIDTH:                                        ; screen width in characters
#org 0x0032     BYTES_PER_ROW:                                ; screen width in bytes
...
; The character set is laid out below in 8 rows of 256 bytes.
; Each row is one row of the displayed character, and each column corresponds to the ASCII code.
; The lowest order bits are the left-most pixels.
; The lowest 8 ASCII codes are used for various constant tables.
...
Font6x8_CharTable:
SetTable:       ,
ClrTable:       ,
RectLeftTable:  ,
RectRightTable: ,
...
; **********************************************************************************************************
; We are at about 0x2917 at this point, leaving 0x6e9 bytes of space
